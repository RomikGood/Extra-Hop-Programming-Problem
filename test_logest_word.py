...
def test_if_no_grid_or_no_words():
	"""test returns if no grid or no word list is provided
	"""
	assert find_longest_word([], []) == 'Please provide valid grid and list of words'
	assert find_longest_word(['A', 'B', 'C'], []) == 'Please provide valid grid and list of words'
	assert find_longest_word([], ['extrahop', 'TCP']) == 'Please provide valid grid and list of words'
...
def test_with_no_words_found_in_grid():
	...
	words = ['algolss', 'fortranss', 'simulass']
